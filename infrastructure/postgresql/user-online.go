...
func (u *userOnlineRepository) GetUserOnlineByConversationID(ctx context.Context, conversationID string) ([]*domain.UserOnline, error) {
	...
	rows, err := u.db.QueryContext(ctx, query, conversationID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (u *userOnlineRepository) CreateUserOnline(ctx context.Context, userOnline *domain.UserOnline) error {
	...
	_, err := u.db.ExecContext(ctx, query, userOnline.ID, userOnline.UserID, userOnline.ConnectionID, userOnline.CreatedAt)
	if err != nil {
		return err
	}
	return nil
...
func NewUserOnlineRepository(db *sql.DB) *userOnlineRepository {
